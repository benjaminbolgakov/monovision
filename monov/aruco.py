...
class Aruco(object):
    ...
    def estimate_bearing(self, tvec, rvec):
        """
        - Calculate bearing of marker relative to camera

        """
        bearing_rad = np.arctan2(tvec[0][0], tvec[2][0])
        bearing_deg = np.rad2deg(bearing_rad)
        return bearing_deg

    def estimate_heading(self, tvec, rvec):
        """
        - Calculate heading/orientation of marker relative to camera
        """
        R_marker_to_cam, _ = cv.Rodrigues(rvec)
        heading_rad = np.arctan2(R_marker_to_cam[0, 2], R_marker_to_cam[2, 2])
        return heading_rad
