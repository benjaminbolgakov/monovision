...
* Outline
** Scope
- _General Computer Vision suite_
  + Camera calibration
  + Aruco detection/classification
  + Pose estimation - heading etc
  + SLAM
  + Facial detection
  + And more ...

- A 'main menu' selecting which program to run.
...
- _Ex 2_
  ...
  *Square size* = 20mm
  + Note:
	input into program as: 11,7 (calibration expects number of 'inner corners', not number of squares)

** Camera Properties
- _Ex 1_
  + camera model: [Andersson WBC 1.1]
...
* Keyframe Data
1. _Pose Estimate (Camera Pose)_
   + A pose consists of 3D coordinates[x,y,z] of the camera, as well
     as the orientation/rotation of the camera, represented by a
     rotation matrix.
   + By identifying and tracking visual features, like corners and
     edges, epipolar geometry and triangulation can be used to estimate
     the relative motion between frames.
   + Tools that can be used:
     + _PnP (Perspective-n-Point)_: Estimates camera pose given 3D points
       and their 2D projections.
     + _Essential Matrix_: Encodes the relative rotation and translation
       between two frames.
   + Illustration: [[./assets/pose-estimation.png]]

2. _Feature Keypoints_
   + 2D coordinates of detected keypoints[ORB].
   + Used for: tracking correspondences, triangulating 3D points and
     reprojecting 3D landmarks.
   + Keypoints consists of descriptors, which is needed for feature
     matching between keyframes, place recognition and loop closure
     detection.

3. _3D Map Point Associations_
   + Links between 2D keypoints in a keyframe and corresponding
     3D landmarks in the global map.
   + Essential for: Reprojection error calculation, bundle adjustment
     and for maintaining map structure.

4. _Camera Intrinsics_
   + Parameters used for projection: Focal length, principal point and
     distortion coefficients.
   + Needed to project 3D points back into image space.

5. _Timestamp_
   + Required for tracking, syncing with other possible data and
     log consistency.

* KVS Integration
** Application Cases
*** Storing Keyframe Data
- _Usefulness_: *[moderate]*
  + Helps with offline debugging, relocalization and loop
    closure if coupled with good frame-indexing logic.
  + Persistance.

- _Performance Gain_: *[low]*

- _Implementation_: *[easy]*
  + Straight serialization of Keyframe attributes.

*** Storing Pose Graph Nodes/Edges - Graph-based SLAM
- _Usefulness_: *[high]*
  + Especially in back-end optimization and large map
    merging scenarios.
  + Enables fast pose retrieval and map consistency validation.

- _Performance Gain_: *[high]*
  + Particularly in large environments or when optimizing offline.

- _Implementation_: *[moderate]*
  + Requires careful structuring of graph data (edges, constraints)
    in KVS.

*** Caching Reprojected Map Points - Depth
- _Usefulness_: *[high]*
  + Particularly for monocular SLAM lacking direct depth info.
  + Can cache triangulated points or multi-view matches for re-use.

- _Performance Gain_: *[high]*
  + Expensive triangulation/matching can be skipped if cached.

- _Implementation_: *[hard]*
  + Requires synchronization with bundle adjustment and map
    management logic.

- _Basic Redis Example_:
  #+begin_src python
    import redis
    import pickle
    r = redis.Redis()
    keyframe_data = {"pose": [1.2, 3.4, 0.5], "image": b"..."}
    r.set("keyframe_42", pickle.dumps(keyframe_data))
  #+end_src

*** Fast Keyframe Retrieval For Loop Closure
- _Usefulness_: *[very high]*
  + Given the SLAM pipeline includes loop closure or place recognition.

- _Performance Gain_: *[very high]*
  + Given it's integrated with a fast place-recognition front-end or
    BoW database.

- _Implementation_: *[moderate]*
  + Especially if using descriptor-based indexing or embedding trees.

*** Persistent Map Storage
- _Usefulness_: *[moderate]*
  + Mostly for long-term SLAM (e.g robotics) where persistence matter.

- _Performance Gain_: *[low]*
  + Mostly for productivity.

- _Implementation_: *[easy]*
  + Serialize poses, descriptors and map points.

** Optional Enhancements
*** Store Numpy Arrays Efficiently
- _For performance, avoid JSON and instead store_:
  + Raw bytes [np.ndarray.tobytes()]
  + Structured binary format [e.g, np.save to buffer]

*** Async Or Queue-Based Logging
- _Mitigates latency_:
  + Batch writes to the KVS using backround threads.

*** LRU Eviction
- Keep a linked list of frames and move-to-front on each hit.
** Example Structure
kvstore/
├── main.c          # Main loop and CLI
├── hashtable.c     # Your hash table implementation
...
*** Caching of Map Data and/or Keyframes *[SLAM]*
*Ref*: [[https://chatgpt.com/share/68415b85-facc-8002-843b-050fee074a2f\\][chatgpt]]
**** Keyframe Data
1. _Pose Estimate (Camera Pose)_
   + A pose consists of 3D coordinates[x,y,z] of the camera, as well
     as the orientation/rotation of the camera, represented by a
...
**** Example Keyframe Datastructure
#+begin_src c++
struct Keyframe {
    int id;
    double timestamp;
    ...
    // Pose
    Eigen::Matrix4d pose; // SE(3) transformation
    ...
    std::vector<MapPoint*> associatedMapPoints;

    // Camera model
    CameraIntrinsics intrinsics;

    // Optional
    cv::Mat imagePyramid[NUM_LEVELS]; // Optional
    BoWVector bowVector; // For place recognition
};
#+end_src
**** Key Considerations
- _No Depth Data_:
  + Monocular vision disables depth data. Instead, triangulation of
    keypoints is used. Storing reliable keypoint matches is therefor
    of critical importance.

- _Scale Ambiguity_
  + All pose estimates and 3D points are up to an arbritary scale,
    be consistent.

- _Relocalization_
  + Keyframes are the basis for recovering from tracking loss, so
    the richer the data, the more robust it is.

** References
[[https://build-your-own.org/redis/03_hello_cs][DIY_REDIS]]
[[https://medium.com/@sakalli.duran/crafting-a-key-value-store-designing-a-hash-table-4a1565329efb][crafting_kvs_hashtable]]
[[https://dev.to/ravikishan/from-theory-to-practice-developing-a-distributed-key-value-database-with-sharding-and-replication-180d][distributed_kvs]]
[[https://blog.algomaster.io/p/designing-a-distributed-key-value-store][designing_distributed_kvs]]
[[https://dev.to/dtxcode/distributed-key-value-store-in-c-c-from-scratch-2j7b][distributed_kvs_c_cpp_from_scratch]]
[[https://github.com/move47/Distributed-Key-Value-Store][distributed_kvs_github]]
[[https://github.com/andy-byers/CalicoDB/tree/main/src][caliodb_example_project]]
[[https://github.com/chxdeng/mabain][mabain_example_project]]
[[https://jacklinweb.github.io/posts/key-value-storage/][kvs_implementation_C]]
* Particle Filter
...
* References
https://www.classes.cs.uchicago.edu/archive/2022/fall/20600-1/particle_filter_project.html#description
http://ais.informatik.uni-freiburg.de/teaching/ws13/mapping/pdf/slam12-fastslam-4.pdf
https://docs.opencv.org/4.7.0/d9/d0c/group__calib3d.html#ga4abc2ece9fab9398f2e560d53c8c9780
https://docs.opencv.org/4.7.0/d9/d0c/group__calib3d.html#gaaae5a7899faa1ffdf268cd9088940248
https://docs.opencv.org/3.4/d4/d2c/group__videostab__motion.html
https://docs.opencv.org/4.7.0/d9/d0c/group__calib3d.html#gad767faff73e9cbd8b9d92b955b50062d
https://github.com/filchy/slam-python/blob/master/extractor.py
https://learnopencv.com/the-complete-guide-to-object-tracking-in-computer-vision/
https://www.ri.cmu.edu/pub_files/pub4/montemerlo_michael_2003_1/montemerlo_michael_2003_1.pdf
https://www.cvl.isy.liu.se/education/undergraduate/tsbb15/
https://www.cvl.isy.liu.se/education/undergraduate/tsbb19/
https://www.cvl.isy.liu.se/education/undergraduate/tsbb21/lectures/
https://gitlab.liu.se/cvl/tsbb19/course-material/-/tree/main
https://www.cvl.isy.liu.se/education/undergraduate/tsbb21/lectures/
https://atsushisakai.github.io/PythonRobotics/modules/slam/FastSLAM1/FastSLAM1.html
https://github.com/yahsiuhsieh/particle-filter-slam/blob/master/src/motion_utils.py
https://github.com/yahsiuhsieh/particle-filter-slam/tree/master
https://stackoverflow.com/questions/71813708/creating-augmented-reality-using-python-opencv-and-slam
https://docs.opencv.org/3.4/dc/dfb/group__datasets__slam.html
https://ltslam-doc.readthedocs.io/en/latest/tutorial/opencv/opencv_tutorial.html
http://avisingh599.github.io/vision/monocular-vo/
https://docs.opencv.org/4.7.0/dc/d2c/tutorial_real_time_pose.html
https://www.youtube.com/watch?v=N451VeA8XRA&ab_channel=NicolaiNielsen
https://github.com/niconielsen32/ComputerVision/blob/master/visual_odometry.py
https://github.com/alishobeiri/Monocular-Video-Odometery/tree/master
